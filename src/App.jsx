...
export default function App() {
  ...
  const generateImage = async () => {
    if (!image || !prompt)
      return alert("Please provide both image and prompt.");
    ...
    reader.onloadend = async () => {
      ...
      try {
        ...
        if (!visionResponse.ok || !visionData.choices?.[0])
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-rose-100 via-purple-100 to-blue-100 relative overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden">
        <div className="absolute -top-40 -right-40 w-150 h-100 bg-purple-300/30 rounded-full mix-blend-multiply filter blur-xl opacity-50 animate-pulse"></div>
        <div className="absolute -bottom-40 -left-40 w-150 h-90 bg-blue-300/30 rounded-full mix-blend-multiply filter blur-xl opacity-50 animate-pulse"></div>
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-200 h-200 bg-rose-700/30 rounded-full mix-blend-multiply filter blur-xl opacity-30 animate-pulse"></div>
      </div>
      ...
      <div className="relative z-10 flex items-center justify-center min-h-screen px-4 py-8">
        <div className="bg-white/70 backdrop-blur-lg p-8 rounded-3xl shadow-lg border border-white/40 w-full max-w-2xl space-y-8">
          ...
          <div className="space-y-3">
            <label className="text-gray-700 font-medium text-sm uppercase tracking-wider">
          ...
          <div className="space-y-3">
            ...
            >
              <option value="1:1" className="bg-white">
                Square (1:1)
              </option>
              <option value="3:2" className="bg-white">
                Landscape (3:2)
              </option>
              <option value="2:3" className="bg-white">
                Portrait (2:3)
              </option>
          ...
          <button
            ...
            className={`w-full relative group overflow-hidden ${
              loading || !image || !prompt ? "cursor-not-allowed" : ""
            }`}
          >
            <div
              className={`absolute inset-0 rounded-2xl transition-opacity duration-300 ${
                loading || !image || !prompt
                  ? "bg-blue-100"
                  : "bg-gradient-to-r from-rose-400 via-purple-400 to-blue-400"
              }`}
            ></div>
            ...
            <div
              className={`absolute inset-0 rounded-2xl blur-sm transition-opacity duration-300 ${
                loading || !image || !prompt
                  ? "opacity-0"
                  : "opacity-0 group-hover:opacity-100 bg-gradient-to-r from-rose-400 via-purple-400 to-blue-400"
              }`}
            ></div>

            <div
              className={`relative text-white py-4 rounded-2xl font-bold text-lg tracking-wide transition-all duration-300 ${
                loading || !image || !prompt
                  ? "bg-blue-200 scale-100"
                  : "bg-gradient-to-r from-rose-400 via-purple-400 to-blue-400 group-hover:scale-105"
              }`}
            >
              {loading ? (
